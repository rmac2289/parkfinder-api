...
const SuggestionsService = {
  getAllSuggestions(db) {
    return db
      ...
      .select("sug.id", "sug.park_name", "sug.location", "sug.description")
      .groupBy("sug.id");
  },

  getById(db, id) {
    return SuggestionsService.getAllSuggestions(db).where("sug.id", id).first();
  },
  insertSuggestions(db, newSuggestions) {
    return db
      ...
      .then((suggestions) => SuggestionsService.getById(db, suggestions.id));
  },
